---
to: <%= resourceRoot %>/<%= h.inflection.transform(name, ['pluralize', 'underscore', 'dasherize']) %>/<%= h.inflection.transform(name, ['pluralize', 'underscore', 'dasherize']) %>.controller.ts
---
import {
  Controller,
  Get,
...
} from '<%= fromResourceToSrc %>utils/dto/infinity-pagination-response.dto';
import { infinityPagination } from '<%= fromResourceToSrc %>utils/infinity-pagination';
